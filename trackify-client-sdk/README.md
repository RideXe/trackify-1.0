...
A Kotlin Multiplatform background location tracking SDK for [Trackify](https://www.trackify.org) - and any other server that accepts the same simple HTTP GET protocol. Runs on Android and iOS, persists positions in a local SQLite queue, and uploads them with network-aware retry.

This repository publishes two artifacts:

- **Native SDK** - Maven Central (`org.trackify:trackify-client-sdk`) and an XCFramework distributed via Swift Package Manager.
- **Flutter plugin** - pub.dev (`trackify_client_sdk`).
...
```kotlin
dependencies {
    implementation("org.trackify:trackify-client-sdk:0.0.11")
}
```
...
```swift
.package(url: "https://github.com/trackify/trackify-client-sdk.git", from: "0.0.11")
```
...
```kotlin
val config = Config(
    serverUrl = "https://demo.trackify.org",
    deviceId = "123456",
)
...
```dart
import 'package:trackify_client_sdk/trackify_client_sdk.dart';

final tracker = TrackifyClientSdk();
...
|---|---|---|---|
| `serverUrl` | `String` | - | Trackify server endpoint (`https://demo.trackify.org`). |
...
- **HttpUploader** - Ktor client; sends each position as an HTTP GET with query parameters (`id`, `lat`, `lon`, `timestamp`, `accuracy`, optionally `altitude`, `speed` in knots, `bearing`, `batt`). This is the OsmAnd-style protocol Trackify consumes; any server that accepts the same params can be the endpoint. Returns success on any 2xx.
...
<array>
    <string>org.trackify.client.heartbeat</string>
</array>
```
