...
* `requestPosition` accepts an optional `alarm` string that becomes the Trackify protocol `alarm` field on the upload (e.g. `"sos"`). One-off remains direct — no buffer/retry — so a failed SOS is not persisted.
