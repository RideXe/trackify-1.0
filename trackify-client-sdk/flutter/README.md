...
Flutter plugin for background location tracking. Sends position updates over a simple HTTP GET protocol - works with [Trackify](https://www.trackify.org) and any compatible server. Wraps the [Trackify Client SDK](https://github.com/trackify/trackify-client-sdk) for Android and iOS - see the main repository for architecture, reliability, and configuration details.
...
If you set `heartbeatIntervalSeconds`, also add `fetch` to `UIBackgroundModes` and add `org.trackify.client.heartbeat` to `BGTaskSchedulerPermittedIdentifiers`. iOS schedules the wake at its discretion.
